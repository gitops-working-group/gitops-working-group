...
However, despite the differences in software systems, several important principles emerge that significantly simplify reliably managing and operating all software systems at scale.
...
By Configuration, we mean data that defines how the system will behave and operate.
This data is distinct and separate from the data the system will process.
...
Firstly, if a defect is perceived in the system's behaviour, it is impossible to determine whether the defect is due to the system having entered an incorrect state, or the defect is in our expectations about the system's behaviour.
...
Secondly, if the desired state of a system is not defined outside of the running system itself, how do we handle a failure that occurs whilst we are modifying its state directly?
...
> All systems managed by GitOps are configured declaratively as data and all resources managed through a GitOps process must be expressed declaratively.
The sum of configuration data for a system forms the _Desired State_ of a system.
